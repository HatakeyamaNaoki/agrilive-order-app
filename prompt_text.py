def get_text_order_prompt():
    """
    SMS/メールテキスト注文解析用のシステムプロンプトを取得
    """
    return """以下のSMS/メール本文（純テキスト）の注文内容を、JSONで構造化してください。
...
### 0. 事前ルール（入力正規化）
- **改行の無視**: 改行は句読点と同様にただの空白として扱い、意味を持たせない（SMS/メールの折返しは無視する）
...
### 1. 基本情報の抽出
- **order_id**: SMS/メール注文では通常空文字
- **order_date**: 発注日（受信日時を基準とする）
- **delivery_date**: 納品希望日（相対表現は受信日時を基準に計算）
- **partner_name**: 顧客名（入力された顧客名を使用）

### 2. 商品情報の抽出
- **商品名**: 商品の正式名称を抽出
- **数量**: 数値のみを抽出（例：「10個」→ quantity: "10"）
- **単位**: 数量の直後の単位を正確に抽出（例：「個」「箱」「束」「kg」「L」など）
- **product_code**: SMS/メール注文では通常空文字
- **unit_price**: SMS/メール注文では通常空文字
- **amount**: SMS/メール注文では通常空文字
...
- **改行の無視**: SMS/メールの行幅制限による改行は論理的な区切りではないため、形容詞の帰属判断では無視する
...
- **SMS/メール特有の表示制約**: SMS/メールアプリの行幅制限により、長いテキストが自動改行されることを考慮する
...
- **改行の論理的解釈**: 画面上の改行位置は、SMS/メールアプリの表示制約によるものであり、注文内容の論理的な区切りではない
...
- **SMS/メール特有の制約理解**: SMS/メールアプリの行幅制限により、長いテキストが自動的に改行されることを理解し、これを考慮した解析を行う
...
- **改行の解釈**: SMS/メールの行幅制限による改行は論理的な区切りではない。形容詞の帰属は画面上の位置ではなく、テキストの論理的な順序で判断する
- **改行後の形容詞**: 改行の直後に形容詞が来た場合、その形容詞は次の商品に帰属する

## 例

### 例1（基本ケース）
入力テキスト：「明日 玉レタス1個 りんご10個 お願いします！」
受信日時：2025/08/01
...
### 例2-2（改行がある場合：SMS/メールの行幅制限による改行）
入力テキスト（改行あり）：
いちじく6
高い
なし6
...
テキストを注意深く分析し、上記の形式で正確なデータを抽出してください。"""
